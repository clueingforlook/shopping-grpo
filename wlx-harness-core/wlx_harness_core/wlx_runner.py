...
from wlx_harness_core.wlx_config import ConfigValidationError, HarnessConfig, validate_config
from wlx_harness_core.wlx_contracts import (
    AssistantTurn,
    EpisodeRequest,
    ErrorCategory,
    HarnessError,
...
from wlx_harness_core.wlx_environment import (
    EnvironmentResult,
...
def _require_structured_initial_observation(initial: EnvironmentResult) -> None:
    ...
    if initial.observation_version != OBSERVATION_VERSION:
        raise StructuredObservationError(
            "WLX SFT requires an initial observation_state rendered as Observation v2"
        )
    if not initial.observation.startswith(STRUCTURED_OBSERVATION_HEADER + "\n"):
        raise StructuredObservationError(
            "WLX SFT initial observation is missing the Observation v2 header"
        )
