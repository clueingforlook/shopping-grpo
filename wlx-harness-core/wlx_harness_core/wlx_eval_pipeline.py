"""WLX Eval v2 离线流水线：已有轨迹 -> 评测、复核队列和 Run 报告。"""
...
from wlx_harness_core.wlx_eval_contracts import (
    WLX_EVAL_METHOD_VERSION,
    WLX_EVAL_OFFLINE_RUN_VERSION,
    WLX_EVAL_SCHEMA_VERSION,
    validate_evaluation_record,
)
from wlx_harness_core.wlx_eval_report import (
    render_summary_markdown,
    summarize_evaluations,
)
from wlx_harness_core.wlx_eval_rubric import validate_rubric_bundle
from wlx_harness_core.wlx_eval_requirements import merge_judged_requirements
from wlx_harness_core.wlx_sft_storage import read_jsonl
from wlx_harness_core.wlx_trajectory_evaluator import evaluate_trajectory
...
class OfflineEvaluationPaths:
    ...
    def __init__(self, output_dir: str | Path) -> None:
        ...
        self.manifest = self.output_dir / "wlx-eval-manifest.json"
        self.evaluations = self.output_dir / "wlx-trajectory-evaluations.jsonl"
        self.review_queue = self.output_dir / "wlx-review-queue.jsonl"
        self.summary_json = self.output_dir / "wlx-run-summary.json"
        self.summary_markdown = self.output_dir / "wlx-run-summary.md"
...
def run_offline_evaluation(
    *,
    ...
    manifest_source = (
        ...
        else source.parent / "wlx-run-manifest.json"
    )
    ...
    for line_number, raw_line in _jsonl_lines(source):
        ...
        evaluation["method_version"] = WLX_EVAL_METHOD_VERSION
    ...
    manifest = {
        "schema_version": WLX_EVAL_OFFLINE_RUN_VERSION,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "run_name": run_name,
        "method_version": WLX_EVAL_METHOD_VERSION,
        "evaluator_schema_version": WLX_EVAL_SCHEMA_VERSION,
        "source": {
    ...
    return {
        "offline_evaluation_version": WLX_EVAL_OFFLINE_RUN_VERSION,
...
def _load_judgments(
    ...
    *,
    ...
    batch_manifest_path = source.parent / "wlx-judge-manifest.json"
...
def _evaluation_versions(
    ...
    return {
        ...
        "offline_evaluator": WLX_EVAL_SCHEMA_VERSION,
        "rubric": None,
        "judge": None,
    }
