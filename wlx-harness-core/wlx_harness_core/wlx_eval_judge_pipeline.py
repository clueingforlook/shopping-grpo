"""WLX Eval Judge 的生成、断点续跑、校准比较和冻结门禁。"""
...
from wlx_harness_core.wlx_eval_judge import (
    WLX_EVAL_JUDGE_PROMPT_VERSION,
    WLX_EVAL_JUDGE_SYSTEM_PROMPT_SHA256,
...
from wlx_harness_core.wlx_eval_rubric_generator import DeepSeekRubricClient
...
WLX_EVAL_JUDGE_BATCH_VERSION = "wlx-eval-v2-judge-batch-v1"
WLX_EVAL_JUDGE_CALIBRATION_VERSION = "wlx-eval-v2-judge-calibration-v1"
...
class JudgeBatchPaths:
    def __init__(self, output_dir: str | Path) -> None:
        ...
        self.root = root
        self.manifest = root / "wlx-judge-manifest.json"
        self.attempts = root / "wlx-judge-attempts.jsonl"
        self.results = root / "wlx-judge-results.jsonl"
        self.review_queue = root / "wlx-judge-review-queue.jsonl"
        self.summary = root / "wlx-judge-summary.json"
...
def judge_case(
    ...
    *,
    ...
    return {
        ...
        "prompt_version": WLX_EVAL_JUDGE_PROMPT_VERSION,
        "prompt_sha256": WLX_EVAL_JUDGE_SYSTEM_PROMPT_SHA256,
        "rubric_requirements": [
            {
...
def run_judge_batch(
    *,
    ...
    contract = {
        "batch_version": WLX_EVAL_JUDGE_BATCH_VERSION,
        "mode": mode,
        "model": client.model,
...
def calibrate_judge(
    *,
    ...
    for result in results.values():
        ...
        if (
            result.get("prompt_version") != WLX_EVAL_JUDGE_PROMPT_VERSION
            or result.get("prompt_sha256") != WLX_EVAL_JUDGE_SYSTEM_PROMPT_SHA256
    ...
    manifest = {
        "schema_version": WLX_EVAL_JUDGE_CALIBRATION_VERSION,
...
def _load_calibration_gate(
    ...
    *,
    ...
    if (
        value.get("schema_version") != WLX_EVAL_JUDGE_CALIBRATION_VERSION
        or value.get("status") != "frozen"
        or value.get("model") != model
        or value.get("prompt_sha256") != WLX_EVAL_JUDGE_SYSTEM_PROMPT_SHA256
    ):
...
__all__ = [
    "JudgeCase",
    "WLX_EVAL_JUDGE_BATCH_VERSION",
    "WLX_EVAL_JUDGE_CALIBRATION_VERSION",
    "calibrate_judge",
    "judge_case",
    "load_judge_cases",
    "run_judge_batch",
