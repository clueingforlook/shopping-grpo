...
from wlx_harness_core.wlx_eval_events import standardize_trajectory_events
from wlx_harness_core.wlx_eval_rubric import validate_rubric_bundle


WLX_EVAL_JUDGE_INPUT_VERSION = "wlx-eval-v2-judge-input-v1"
WLX_EVAL_JUDGE_OUTPUT_VERSION = "wlx-eval-v2-judge-process-output-v1"
WLX_EVAL_JUDGE_FULL_OUTPUT_VERSION = "wlx-eval-v2-judge-output-v1"
WLX_EVAL_JUDGE_PROMPT_VERSION = "wlx-eval-v2-judge-prompt-v2"
FAILURE_TYPES = {
...
def build_process_judge_payload(
    *,
    ...
    return {
        "schema_version": WLX_EVAL_JUDGE_INPUT_VERSION,
...
Prompt 版本：{WLX_EVAL_JUDGE_PROMPT_VERSION}"""
    return [
...
WLX_EVAL_JUDGE_SYSTEM_PROMPT_SHA256 = hashlib.sha256(
...
def validate_process_judgment(
    ...
    *,
    ...
    if value.get("schema_version") != WLX_EVAL_JUDGE_OUTPUT_VERSION:
...
def validate_full_judgment(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise TypeError("judge output must be an object")
    if value.get("schema_version") != WLX_EVAL_JUDGE_FULL_OUTPUT_VERSION:
