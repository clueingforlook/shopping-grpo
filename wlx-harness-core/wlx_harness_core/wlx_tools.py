...
from wlx_harness_core.wlx_contracts import TOOL_SCHEMA_VERSION, ToolAction, ToolCall
...
WLX_SFT_TOOL_SCHEMAS = tuple(
    deepcopy(schema)
    for schema in SHOP_TOOL_SCHEMAS
    if (schema.get("function") or {}).get("name") != "think"
)
WLX_SFT_TOOL_REGISTRY = ToolRegistry(WLX_SFT_TOOL_SCHEMAS)
