...
from wlx_harness_core.wlx_eval_contracts import PROCESS_DIMENSIONS
...
from wlx_harness_core.wlx_sft_storage import file_sha256, read_jsonl, write_jsonl
from wlx_harness_core.wlx_trajectory_evaluator import evaluate_trajectory


WLX_JUDGE_GOLD_VERSION = "wlx-eval-judge-gold-v1"
...
def build_judge_gold_draft(
    *,
    ...
    paths = {
        "manifest": output / "wlx-judge-gold-manifest.json",
        "labels": output / "wlx-judge-gold-labels-draft.jsonl",
        "markdown": output / "wlx-judge-gold-review.md",
    }
    ...
    manifest = {
        "schema_version": WLX_JUDGE_GOLD_VERSION,
...
def _render_header(manifest: Mapping[str, Any]) -> str:
    return f"""# WLX Judge Gold Set 人工确认
...
__all__ = ["WLX_JUDGE_GOLD_VERSION", "build_judge_gold_draft"]
