"""把任务检查、WLX Core 采样和 SFT 数据构建串成一条清楚的阶段流水线。"""
...
from wlx_harness_core.wlx_config import ContextPolicy, HarnessConfig, ObservationPolicy
from wlx_harness_core.wlx_runner import EpisodeRunner
from wlx_harness_core.wlx_sft_contracts import SamplingConfig, SftTask
from wlx_harness_core.wlx_sft_dataset import DatasetBuildConfig, build_dataset_artifacts
from wlx_harness_core.wlx_sft_metrics import TrainingTokenCounter
from wlx_harness_core.wlx_sft_prompt import (
    WLX_SFT_INITIAL_MESSAGE_VERSION,
    WLX_SFT_SYSTEM_PROMPT,
    WLX_SFT_SYSTEM_PROMPT_SHA256,
    WLX_SFT_SYSTEM_PROMPT_VERSION,
)
from wlx_harness_core.wlx_sft_sampler import CoreAttemptExecutor, SftSamplingScheduler
...
from wlx_harness_core.wlx_tools import WLX_SFT_TOOL_REGISTRY
...
class SftBatchPaths:
    ...
    def under(cls, batch_dir: str | Path) -> "SftBatchPaths":
        ...
        return cls(
            ...
            dataset_dir=root / "wlx-dataset-v1",
        )
...
class WlxSftDataPipeline:
    ...
    async def collect(
        ...
        *,
        ...
        """用 WLX Core 完成在线采样；模型与环境配置正确后可随时断点续采。"""
...
def sft_harness_contract(
    ...
    return {
        ...
        "runner": {
            ...
            "initial_message_version": WLX_SFT_INITIAL_MESSAGE_VERSION,
...
def _assert_sft_runner_contract(runner: EpisodeRunner) -> None:
    ...
    WLX_SFT_TOOL_REGISTRY.assert_compatible(runner.tool_registry)
    if runner.system_prompt != WLX_SFT_SYSTEM_PROMPT:
        raise ValueError("SFT runner 必须使用冻结的 WLX_SFT_SYSTEM_PROMPT")
