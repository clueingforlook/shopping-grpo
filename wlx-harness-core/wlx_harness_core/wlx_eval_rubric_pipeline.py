"""WLX Rubric 批量生成、断点续跑、复核队列和最终冻结。"""
...
from wlx_harness_core.wlx_eval_rubric import (
    freeze_rubric_bundle,
    validate_rubric_bundle,
)
from wlx_harness_core.wlx_eval_rubric_generator import (
    DeepSeekRubricClient,
    RubricGenerationError,
    RubricTask,
    WLX_RUBRIC_GENERATOR_VERSION,
    WLX_RUBRIC_PROMPT_VERSION,
    WLX_RUBRIC_SYSTEM_PROMPT_SHA256,
...
WLX_RUBRIC_BATCH_VERSION = "wlx-rubric-batch-v1"
WLX_RUBRIC_MANIFEST_VERSION = "wlx-rubric-generation-manifest-v1"
WLX_RUBRIC_FINALIZATION_VERSION = "wlx-rubric-finalization-v1"
...
class RubricBatchPaths:
    ...
    def __init__(self, output_dir: str | Path) -> None:
        ...
        self.manifest = self.output_dir / "wlx-rubric-generation-manifest.json"
        self.attempts = self.output_dir / "wlx-rubric-attempts.jsonl"
        self.candidates = self.output_dir / "wlx-rubric-candidates.jsonl"
        self.auto_frozen = self.output_dir / "wlx-rubrics-auto-frozen.jsonl"
        self.review_queue = self.output_dir / "wlx-rubric-review-queue.jsonl"
        self.review_decisions_template = (
            self.output_dir / "wlx-rubric-review-decisions-template.jsonl"
        )
        self.summary = self.output_dir / "wlx-rubric-generation-summary.json"
        self.final_rubrics = self.output_dir / "wlx-rubrics.jsonl"
        self.applied_reviews = self.output_dir / "wlx-rubric-review-decisions-applied.jsonl"
        self.finalization_manifest = (
            self.output_dir / "wlx-rubric-finalization-manifest.json"
        )
...
def run_rubric_generation(
    *,
    ...
    contract = {
        "batch_version": WLX_RUBRIC_BATCH_VERSION,
        "generator_version": WLX_RUBRIC_GENERATOR_VERSION,
        "prompt_version": WLX_RUBRIC_PROMPT_VERSION,
        "prompt_sha256": WLX_RUBRIC_SYSTEM_PROMPT_SHA256,
...
def finalize_rubric_set(
    *,
    ...
    final_manifest = {
        "schema_version": WLX_RUBRIC_FINALIZATION_VERSION,
    ...
    return {
        "finalization_version": WLX_RUBRIC_FINALIZATION_VERSION,
        "tasks": len(rubrics),
        "human_reviewed": len(applied),
        "rubrics": str(paths.final_rubrics),
...
def _prepare_generation_manifest(
    ...
    if paths.manifest.exists():
        manifest = _read_json_object(paths.manifest)
        if (
            manifest.get("schema_version") != WLX_RUBRIC_MANIFEST_VERSION
    ...
    manifest = {
        "schema_version": WLX_RUBRIC_MANIFEST_VERSION,
        "created_at": datetime.now(timezone.utc).isoformat(),
...
__all__ = [
    ...
    "RubricBatchPaths",
    "WLX_RUBRIC_BATCH_VERSION",
    "WLX_RUBRIC_FINALIZATION_VERSION",
    "WLX_RUBRIC_MANIFEST_VERSION",
    "finalize_rubric_set",
    "load_rubric_tasks",
