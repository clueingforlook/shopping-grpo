...
for path in (REPOSITORY / "src", REPOSITORY / "wlx-harness-core", REPOSITORY):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from wlx_harness_core.wlx_config import HarnessConfig  # noqa: E402
from wlx_harness_core.wlx_contracts import EpisodeRequest  # noqa: E402
...
_SCRIPT_PATH = REPOSITORY / "scripts" / "wlx_evaluate_trajectories.py"
_SCRIPT_SPEC = importlib.util.spec_from_file_location(
    "wlx_evaluate_trajectories_script",
    _SCRIPT_PATH,
)
if _SCRIPT_SPEC is None or _SCRIPT_SPEC.loader is None:  # pragma: no cover
    raise RuntimeError(f"无法加载 WLX 评测入口：{_SCRIPT_PATH}")
...
class TrajectoryEvaluationTest(unittest.TestCase):
    ...
    def test_jsonl_entrypoint_writes_one_result_per_trajectory(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = root / "wlx-trajectories.jsonl"
            output = root / "wlx-evaluation.jsonl"
            source.write_text(
