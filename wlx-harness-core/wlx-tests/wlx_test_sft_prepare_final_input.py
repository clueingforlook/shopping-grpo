...
SCRIPT_PATH = REPOSITORY / "scripts" / "wlx_sft_prepare_final_input.py"
SPEC = importlib.util.spec_from_file_location("wlx_sft_prepare_final_input", SCRIPT_PATH)
...
def _plan_row(task_id: int, label: str) -> dict:
    return {
        ...
        "difficulty_version": "wlx-difficulty-final-test",
    }
    ...
    return {
        ...
        "stage_metadata": {
            "request": {
                ...
                "difficulty_version": "wlx-old-label",
...
class PrepareFinalInputTest(unittest.TestCase):
    def test_copies_rows_and_applies_final_difficulty_without_touching_sources(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            raw_a = root / "wlx-raw-a.jsonl"
            raw_b = root / "wlx-raw-b.jsonl"
            plan = root / "wlx-plan.jsonl"
            held_out = root / "wlx-evaluation.jsonl"
            output = root / "wlx-output"
            ...
            self.assertEqual(request["difficulty_version"], "wlx-difficulty-final-test")
