"""WLX Rubric 的规则、DeepSeek 客户端、批处理和最终冻结测试。"""
...
class RubricBatchTest(unittest.TestCase):
    ...
    def test_batch_is_resumable_and_finalization_requires_reviews(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            rubric_rows = [
                json.loads(line)
                for line in (output / "wlx-rubrics.jsonl")
...
class RubricCliSafetyTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        path = REPOSITORY / "scripts" / "wlx_generate_eval_rubrics.py"
        spec = importlib.util.spec_from_file_location("wlx_generate_eval_rubrics", path)
