#!/usr/bin/env python3
"""把现有 WLX 原始轨迹和最终 SFT 冻结为两类清晰、可审计的数据。"""
...
DEFAULT_OUTPUT = REPOSITORY / "outputs" / "wlx-data-v1"
HELD_OUT_TASKS = REPOSITORY / "data" / "evaluation" / "tasks.jsonl"

CALIBRATION_CANONICAL = Path(
    "outputs/wlx-sft-calibration-final-v2/wlx-calibration-raw.jsonl"
)
CALIBRATION_PLAN = Path(
    "outputs/wlx-sft-calibration-final-v2/wlx-calibration-plan.jsonl"
)
CALIBRATION_MERGE_MANIFEST = Path(
    "outputs/wlx-sft-calibration-final-v2/wlx-calibration-merge-manifest.json"
)
CALIBRATION_BATCHES = (
    "wlx-sft-calibration-smoke-c2-v1",
    "wlx-sft-calibration-smoke-c2-v2",
    "wlx-sft-calibration-smoke-c2-v3",
    "wlx-sft-calibration-smoke-c4-v1",
    "wlx-sft-calibration-c4-remaining194-v1",
    "wlx-sft-calibration-replacements-c4-v1",
)
FORMAL_BATCHES = (
    "wlx-sft-formal-batch-001-v1",
    "wlx-sft-formal-hard100-run-v1",
    "wlx-sft-formal-easy63-medium256-run-v1",
    "wlx-sft-formal-easy-topup-run-v1",
    "wlx-sft-formal-medium-topup-run-v1",
    "wlx-sft-formal-hard-topup-run-v1",
)
FORMAL_PLAN_DIRS = (
    "wlx-sft-formal-batch-v1",
    "wlx-sft-formal-hard100-v1",
    "wlx-sft-formal-easy63-medium256-v1",
    "wlx-sft-formal-easy-topup-v1",
    "wlx-sft-formal-medium-topup-v1",
    "wlx-sft-formal-hard-topup-v1",
)
FINAL_DIRECTORY = Path("outputs/wlx-sft-final-v1")
FINAL_FILES = (
    "wlx-sft.jsonl",
    "wlx-train.jsonl",
    "wlx-validation.jsonl",
    "wlx-accepted.jsonl",
    "wlx-rejected.jsonl",
    "wlx-alternatives.jsonl",
    "wlx-metadata.json",
    "wlx-reject-stats.json",
)
FINAL_DIFFICULTY_PLAN = Path(
    "outputs/wlx-sft-formal-plan-v1/wlx-formal-all-v3.jsonl"
)
DIFFICULTY_PROVENANCE = Path("outputs/wlx-sft-difficulty-final-v3")
FILTER_MERGE_MANIFEST = Path(
    "outputs/wlx-sft-filter-input-v1/wlx-merge-manifest.json"
)
TASK_PLAN_PROVENANCE = Path("outputs/wlx-sft-task-plan-v1")
...
def build_archive(output_dir: Path) -> dict[str, Any]:
    ...
    temporary = Path(
        tempfile.mkdtemp(prefix="wlx-data-v1-tmp-", dir=output_dir.parent)
    )
    try:
...
def _build_archive_tree(actual_root: Path, logical_root: Path) -> None:
    raw_root = actual_root / "wlx-raw"
    calibration_root = raw_root / "wlx-calibration-200"
    formal_root = raw_root / "wlx-formal"
    sft_root = actual_root / "wlx-sft"
    ...
    raw_records_path = raw_root / "wlx-records.jsonl"
    ...
    raw_manifest = {
        ...
        "files": _tree_reports(
            ...
            logical_root=logical_root / "wlx-raw",
            exclude_names={"wlx-manifest.json"},
        ),
    }
    _write_json(raw_root / "wlx-manifest.json", raw_manifest)

    root_manifest = {
        "archive_version": ARCHIVE_VERSION,
        "layout": {
            "wlx-raw": "全部原始轨迹及逐条结果记录",
            "wlx-sft": "最终 SFT 数据及逐条接收/拒绝记录",
        },
        "summary": {
        ...
        "manifests": [
            {
                **_file_report(
                    raw_root / "wlx-manifest.json",
                    logical_root / "wlx-raw" / "wlx-manifest.json",
                ),
                "archive_relative_path": "wlx-raw/wlx-manifest.json",
            },
            {
                **_file_report(
                    sft_root / "wlx-manifest.json",
                    logical_root / "wlx-sft" / "wlx-manifest.json",
                ),
                "archive_relative_path": "wlx-sft/wlx-manifest.json",
            },
        ],
    }
    _write_json(actual_root / "wlx-manifest.json", root_manifest)
...
def _build_calibration(
    ...
    canonical_output = output / "wlx-raw.jsonl"
    shutil.copy2(canonical_source, canonical_output)

    locators: dict[str, dict[str, Any]] = {}
    source_reports: list[dict[str, Any]] = []
    for batch in CALIBRATION_BATCHES:
        raw_path = _source(Path("outputs") / batch / "wlx-raw.jsonl")
    ...
    logical_canonical = logical_root / "wlx-raw" / "wlx-calibration-200" / "wlx-raw.jsonl"
    ...
    extras_output = output / "wlx-extra-history.jsonl"
    extra_records: list[dict[str, Any]] = []
    logical_extras = (
        logical_root
        / "wlx-raw"
        / "wlx-calibration-200"
        / "wlx-extra-history.jsonl"
    )
    ...
    _write_jsonl(output / "wlx-records.jsonl", records)
    provenance = output / "wlx-provenance"
    provenance.mkdir()
    shutil.copy2(_source(CALIBRATION_PLAN), provenance / "wlx-calibration-plan.jsonl")
    shutil.copy2(
        _source(CALIBRATION_MERGE_MANIFEST),
        provenance / "wlx-calibration-merge-manifest.json",
    )
    for source_path in sorted(_source(TASK_PLAN_PROVENANCE).iterdir()):
        if source_path.is_file() and source_path.name.startswith("wlx-"):
    ...
    manifest = {
        ...
        "description": (
            "wlx-raw.jsonl 是 200 个校准任务的 canonical 942 行；"
            "wlx-extra-history.jsonl 单列未进入 canonical 的 49 条历史轨迹"
        ),
        ...
        "files": _tree_reports(
            actual_root=output,
            logical_root=logical_root / "wlx-raw" / "wlx-calibration-200",
            exclude_names={"wlx-manifest.json"},
        ),
    }
    _write_json(output / "wlx-manifest.json", manifest)
...
def _build_formal(
    ...
    merged_output = output / "wlx-raw.jsonl"
    logical_raw = logical_root / "wlx-raw" / "wlx-formal" / "wlx-raw.jsonl"
    ...
    for directory_name in FORMAL_PLAN_DIRS:
        ...
        for source_path in sorted(directory.iterdir()):
            if source_path.is_file() and source_path.name.startswith("wlx-"):
                shutil.copy2(source_path, provenance / source_path.name)
    for batch in FORMAL_BATCHES:
        log_path = _source(Path("outputs") / batch / "wlx-collector.log")
        if log_path.exists():
            shutil.copy2(
                log_path,
    ...
    shutil.copy2(
        ...
        provenance / "wlx-formal-all-v3.jsonl",
    )
    ...
    manifest = {
        ...
        "files": _tree_reports(
            actual_root=output,
            logical_root=logical_root / "wlx-raw" / "wlx-formal",
            exclude_names={"wlx-manifest.json"},
        ),
    }
    _write_json(output / "wlx-manifest.json", manifest)
...
def _build_sft(
    ...
    _write_jsonl(output / "wlx-records.jsonl", evaluated_records)

    provenance = output / "wlx-provenance"
    provenance.mkdir()
    shutil.copy2(
        _source(FILTER_MERGE_MANIFEST),
        provenance / "wlx-filter-input-merge-manifest.json",
    )
    for source_path in sorted(_source(DIFFICULTY_PROVENANCE).iterdir()):
        if source_path.is_file() and source_path.name.startswith("wlx-"):
            shutil.copy2(source_path, provenance / source_path.name)
    ...
    manifest = {
        ...
        "historical_metadata_note": (
            "wlx-metadata.json 原样保留生成时路径；整理后的定位以本 manifest 为准"
        ),
        "summary": {
        ...
        "files": _tree_reports(
            actual_root=output,
            logical_root=logical_root / "wlx-sft",
            exclude_names={"wlx-manifest.json"},
        ),
    }
    _write_json(output / "wlx-manifest.json", manifest)
...
def _load_final_decisions() -> dict[str, Any]:
    ...
    accepted = _load_unique_rows(directory / "wlx-accepted.jsonl", "trajectory_id")
    rejected = _load_unique_rows(directory / "wlx-rejected.jsonl", "trajectory_id")
    sft = _load_unique_rows(directory / "wlx-sft.jsonl", "trajectory_id")
    train = _load_unique_rows(directory / "wlx-train.jsonl", "trajectory_id")
    validation = _load_unique_rows(directory / "wlx-validation.jsonl", "trajectory_id")
    alternatives = _load_unique_rows(directory / "wlx-alternatives.jsonl", "trajectory_id")
...
def verify_archive(
    ...
    *,
    ...
    raw_root = output_dir / "wlx-raw"
    calibration = raw_root / "wlx-calibration-200"
    formal = raw_root / "wlx-formal"
    sft = output_dir / "wlx-sft"
    required = (
        output_dir / "wlx-manifest.json",
        raw_root / "wlx-records.jsonl",
        calibration / "wlx-raw.jsonl",
        calibration / "wlx-extra-history.jsonl",
        calibration / "wlx-records.jsonl",
        formal / "wlx-raw.jsonl",
        formal / "wlx-records.jsonl",
        sft / "wlx-sft.jsonl",
        sft / "wlx-train.jsonl",
        sft / "wlx-validation.jsonl",
        sft / "wlx-accepted.jsonl",
        sft / "wlx-rejected.jsonl",
        sft / "wlx-alternatives.jsonl",
        sft / "wlx-records.jsonl",
    )
    for path in required:
        _require_file(path)

    canonical = _load_unique_rows(calibration / "wlx-raw.jsonl", "trajectory_id")
    extras = _load_unique_rows(calibration / "wlx-extra-history.jsonl", "trajectory_id")
    formal_rows = _load_unique_rows(formal / "wlx-raw.jsonl", "trajectory_id")
    raw_records = _load_unique_rows(raw_root / "wlx-records.jsonl", "trajectory_id")
    calibration_records = _load_unique_rows(calibration / "wlx-records.jsonl", "trajectory_id")
    formal_records = _load_unique_rows(formal / "wlx-records.jsonl", "trajectory_id")
    accepted = _load_unique_rows(sft / "wlx-accepted.jsonl", "trajectory_id")
    rejected = _load_unique_rows(sft / "wlx-rejected.jsonl", "trajectory_id")
    sft_rows = _load_unique_rows(sft / "wlx-sft.jsonl", "trajectory_id")
    train = _load_unique_rows(sft / "wlx-train.jsonl", "trajectory_id")
    validation = _load_unique_rows(sft / "wlx-validation.jsonl", "trajectory_id")
    alternatives = _load_unique_rows(sft / "wlx-alternatives.jsonl", "trajectory_id")
    selection_records = _load_unique_rows(sft / "wlx-records.jsonl", "trajectory_id")
    ...
    for manifest_path in (
        calibration / "wlx-manifest.json",
        formal / "wlx-manifest.json",
        raw_root / "wlx-manifest.json",
        sft / "wlx-manifest.json",
    ):
        _verify_manifest_files(output_dir, manifest_path)
        if verify_sources:
            _verify_manifest_sources(manifest_path)
    root_manifest = json.loads(
        (output_dir / "wlx-manifest.json").read_text(encoding="utf-8")
    )
...
def _copy_batch_configs(batch_names: Sequence[str], target: Path) -> None:
    for batch in batch_names:
        source_path = _source(Path("outputs") / batch / "wlx-collection-config.json")
        shutil.copy2(
            source_path,
...
def _all_required_sources() -> list[Path]:
    ...
    for batch in CALIBRATION_BATCHES + FORMAL_BATCHES:
        paths.extend(
            [
                _source(Path("outputs") / batch / "wlx-raw.jsonl"),
                _source(Path("outputs") / batch / "wlx-collection-config.json"),
            ]
        )
        optional_log = _source(Path("outputs") / batch / "wlx-collector.log")
