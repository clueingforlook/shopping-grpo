...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--run-manifest",
        type=Path,
        help="默认自动读取轨迹同目录的 wlx-run-manifest.json",
    )
    parser.add_argument(
        "--rubrics",
        type=Path,
        help="已冻结的 wlx-rubrics.jsonl；提供后启用正式 Rubric 需求检查",
    )
...
def run_from_args(args: argparse.Namespace) -> dict:
    output_dir = args.output_dir or args.trajectories.parent / "wlx-eval-v2"
