#!/usr/bin/env python3
"""WLX Reward v4 GRPO launcher with preflight and safe resume."""
...
DEFAULT_SFT_ADAPTER = ROOT / "outputs/models/wlx-sft-own-data-v1-lora"
DEFAULT_MERGED_MODEL = ROOT / "outputs/models/wlx-sft-own-data-v1-merged"
DEFAULT_DATA_DIR = ROOT / "data/wlx-grpo"
DEFAULT_OUTPUT_ROOT = ROOT / "outputs/models"
DEFAULT_CONFIG = ROOT / "configs/wlx_grpo.yaml"
DEFAULT_AGENT_CONFIG = ROOT / "configs/wlx_agent_loop.yaml"
DEFAULT_TOOL_CONFIG = ROOT / "configs/tools.json"
DEFAULT_MANIFEST = ROOT / "data/wlx-environment.json"
RUN_MANIFEST = "wlx-run-manifest.json"
TRAIN_LOG = "wlx-train.log"
...
def _run_name(value: str) -> str:
    if not re.fullmatch(r"wlx[-_][A-Za-z0-9._-]+", value):
        raise argparse.ArgumentTypeError("run-name 必须以 wlx- 或 wlx_ 开头")
    return value
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-name", type=_run_name, default="wlx-rl-v4-main-24k-noentropy"
    )
...
def _prepare_data(data_dir: Path) -> tuple[Path, Path, Path]:
    train = data_dir / "wlx-train.parquet"
    validation = data_dir / "wlx-validation.parquet"
    metadata = data_dir / "wlx-metadata.json"
    present = [path.is_file() for path in (train, validation, metadata)]
    if any(present) and not all(present):
        raise SystemExit(f"WLX 任务集不完整，请检查：{data_dir}")
    if not all(present):
        command = [sys.executable, str(ROOT / "scripts/wlx_prepare_rl_data.py"), "--output-dir", str(data_dir)]
        subprocess.run(command, cwd=ROOT, check=True)
    recorded = json.loads(metadata.read_text(encoding="utf-8"))
    if recorded.get("schema_version") != "wlx-grpo-data-v1":
        raise SystemExit(f"WLX 任务元数据版本不正确：{metadata}")
    if recorded["train"]["sha256"] != _sha256(train) or recorded["validation"]["sha256"] != _sha256(validation):
        raise SystemExit("WLX 任务文件与冻结元数据哈希不一致")
...
def _environment(args: argparse.Namespace, model: Path, train: Path, validation: Path, output: Path) -> dict[str, str]:
    ...
    environment.update(
        {
            ...
            "WLX_SHOPPING_TOOL_CONFIG": str(DEFAULT_TOOL_CONFIG),
            "WLX_SHOPPING_AGENT_LOOP_CONFIG": str(DEFAULT_AGENT_CONFIG),
            "WLX_GRPO_MODEL_PATH": str(model),
            "WLX_GRPO_TRAIN_FILE": str(train),
            "WLX_GRPO_VAL_FILE": str(validation),
            "WLX_GRPO_OUTPUT_DIR": str(output),
            # The pinned preflight still reads these compatibility names.
    ...
    if args.logger == "swanlab":
        environment.update({"SWANLAB_MODE": "online", "SWANLAB_LOG_DIR": str(output / "wlx-swanlab")})
    return environment
...
def _fingerprint(model: Path, train: Path, validation: Path, metadata: Path, overrides: list[str]) -> dict[str, Any]:
    sources = [
        ...
        DEFAULT_TOOL_CONFIG,
        DEFAULT_MANIFEST,
        ROOT / "src/shopping_grpo/training/grpo/wlx_reward_v4.py",
        ROOT / "src/shopping_grpo/training/grpo/wlx_step_grpo.py",
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"WLX RL 预检查失败（exit={exc.returncode}）") from None
    if args.mode == "preflight":
        print("WLX RL 预检查通过；没有启动训练。")
        return
    ...
    print(f"WLX RL mode={args.mode} | output={output}")
