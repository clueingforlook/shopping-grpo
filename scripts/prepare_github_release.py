#!/usr/bin/env python3
"""为 GitHub Release 生成去除机器路径的 Raw/SFT 附件。"""
...
import sft_organize_data as organizer


REPOSITORY = Path(__file__).resolve().parents[1]
DEFAULT_SOURCE = REPOSITORY / "outputs" / "data-v1"
DEFAULT_OUTPUT = REPOSITORY / "outputs" / "github-release-v1"
RELEASE_BUNDLE_VERSION = "wlx-github-release-bundle-v1"
RAW_ASSET = "raw-v1.tar.gz"
SFT_ASSET = "sft-v1.tar.gz"
...
def build_release_bundle(*, source_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    staging_parent = Path(
        tempfile.mkdtemp(prefix="release-staging-", dir=output_dir.parent)
    )
    temporary_output = Path(
        tempfile.mkdtemp(prefix="release-output-", dir=output_dir.parent)
    )
    staging = staging_parent / "data-v1"
    try:
        ...
        _write_asset(staging, "raw", raw_path)
        _write_asset(staging, "sft", sft_path)
...
def _refresh_manifests(root: Path, replacements: dict[str, int]) -> None:
    logical_root = Path("outputs/data-v1")
    groups = (
        (root / "raw" / "calibration-200", logical_root / "raw" / "calibration-200"),
        (root / "raw" / "formal", logical_root / "raw" / "formal"),
        (root / "raw", logical_root / "raw"),
        (root / "sft", logical_root / "sft"),
    )
    release_note = {
    ...
    for actual_root, logical_group in groups:
        manifest_path = actual_root / "manifest.json"
    ...
    raw_report["archive_relative_path"] = "raw/manifest.json"
    sft_report = organizer._file_report(
        root / "sft" / "manifest.json",
        logical_root / "sft" / "manifest.json",
    )
    sft_report["archive_relative_path"] = "sft/manifest.json"
    ...
    with output.open("wb") as raw_handle:
        with gzip.GzipFile(
            ...
            with tarfile.open(fileobj=gzip_handle, mode="w", format=tarfile.PAX_FORMAT) as archive:
                ...
                _add_tar_path(archive, root / "manifest.json", root, recursive=False)
...
def _add_tar_path(
    ...
    *,
    ...
    arcname = Path("data-v1") / path.relative_to(root)
...
def _verify_assets(
    ...
    for report in assets:
        ...
        with tarfile.open(path, "r:gz") as archive:
            ...
            if "data-v1/manifest.json" not in names:
