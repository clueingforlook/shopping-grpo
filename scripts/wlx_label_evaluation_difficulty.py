#!/usr/bin/env python3
"""用冻结的 WLX 难度模型给 Final-200 添加难度标签。
...
from wlx_sft_task_planner import build_task_features  # noqa: E402
...
DEFAULT_MODEL = (
    REPOSITORY
    / "outputs/wlx-data-v1/wlx-sft/wlx-provenance/wlx-difficulty-model.json"
)
DEFAULT_STATS = (
    REPOSITORY
    / "outputs/wlx-data-v1/wlx-raw/wlx-calibration-200/wlx-provenance"
    / "wlx-difficulty-stats.json"
)
DEFAULT_FIT_REPORT = (
    REPOSITORY
    / "outputs/wlx-data-v1/wlx-sft/wlx-provenance/wlx-difficulty-report.json"
)
DEFAULT_OUTPUT_DIR = REPOSITORY / "outputs/wlx-evaluation-difficulty-v1"
...
def label_evaluation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = {
        ...
        "interpretation": (
            "predicted DeepSeek V4 Flash success under the frozen WLX SFT "
    ...
    labels_path = args.output_dir / "wlx-evaluation-difficulty.jsonl"
    summary_path = args.output_dir / "wlx-evaluation-difficulty-summary.json"
    _atomic_write(
        labels_path,
        "".join(
