...
from shopping_grpo.harness.sft_contracts import (  # noqa: E402
    SamplingConfig,
    SamplingMode,
)
from shopping_grpo.harness.sft_dataset import DatasetBuildConfig  # noqa: E402
...
from shopping_grpo.harness.sft_policy import OpenAICompatibleTeacherPolicy  # noqa: E402
...
from shopping_grpo.harness.sft_tasks import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    ...
    check.add_argument(
        ...
        type=Path,
        default=(
            Path(os.environ["DEEPSEEK_API_KEY_FILE"])
            if os.environ.get("DEEPSEEK_API_KEY_FILE")
            else None
        ),
    )
    check.add_argument(
    ...
    collect = subparsers.add_parser(
        "collect",
        help="使用 Harness Core 在线采集 calibration 或 formal 轨迹",
    )
    ...
    build.add_argument(
        "--training-tokenizer",
        default=os.environ.get("TRAINING_TOKENIZER", "Qwen/Qwen3.5-2B"),
    )
...
def _precompute_difficulty(args: argparse.Namespace) -> int:
    ...
    cache_path = output_dir / "retrieval-cache.jsonl"
    try:
        precompute_retrieval_cache(
            goals=goals,
    ...
    write_public_task_plan(output_dir / "public-train-tasks.jsonl", public_tasks)
    ...
    write_jsonl(
        output_dir / "difficulty-features.jsonl",
        (item.to_dict() for item in features),
    )
    _write_json(
        output_dir / "difficulty-stats.json",
...
def _fit_difficulty(args: argparse.Namespace) -> int:
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    _write_json(output_dir / "difficulty-model.json", model.to_dict())
    _write_json(output_dir / "difficulty-report.json", report)
...
async def _collect(args: argparse.Namespace) -> int:
    ...
    missing = [
        ...
        for name, value in (
            ...
            ("--teacher-tokenizer 或 TEACHER_TOKENIZER", args.teacher_tokenizer),
        )
        if not value
    ]
    if missing:
    ...
    pipeline = SftDataPipeline(harness_config=harness_config)
    ...
    metadata_path = Path(args.raw).with_name("collection-config.json")
    ...
    if metadata_path.exists():
        ...
        raise SystemExit(
            "raw 文件已有内容但缺少 collection-config.json；"
