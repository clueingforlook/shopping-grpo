...
EXPECTED_PATCHED_SHA256 = "a6877cf83b0908714a37a7744e2ad294bf12f31985f1ad102d48057694c943a4"
LEGACY_PATCHED_SHA256 = "fc3564cc5680a9fa92ca7b0a9bc3ae87ccdc90c498ab1bfe34c6796d6c54fb5a"
...
def resolve_installed_ray_trainer() -> Path:
    ...
    # Resolve the package without importing ``verl``. Its top-level module imports
    # torch through ``DataProto``, which can exceed small setup-container memory
    # limits even though applying this source patch does not need torch at all.
    verl_source = Path(distribution.locate_file("verl/__init__.py")).resolve()
...
def apply_patch(target: Path) -> None:
    ...
    if target_hash == LEGACY_PATCHED_SHA256:
        if not backup.is_file() or sha256(backup) != EXPECTED_ORIGINAL_SHA256:
            raise RuntimeError("cannot migrate legacy patch without its verified original backup")
        shutil.copy2(backup, target)
        target_hash = EXPECTED_ORIGINAL_SHA256
        print(f"restored verified original before migrating legacy patch: {target}")
