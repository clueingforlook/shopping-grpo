#!/usr/bin/env python3
"""Build the frozen 20/60/20 WLX GRPO task split from existing tasks."""
...
for path in (ROOT / "src", ROOT / "wlx-harness-core"):
...
DEFAULT_PUBLIC_TASKS = (
    ROOT
    / "outputs/wlx-data-v1/wlx-raw/wlx-calibration-200/wlx-provenance"
    / "wlx-public-train-tasks.jsonl"
)
DEFAULT_FEATURES = DEFAULT_PUBLIC_TASKS.with_name("wlx-difficulty-features.jsonl")
DEFAULT_MODEL = (
    ROOT / "outputs/wlx-data-v1/wlx-sft/wlx-provenance/wlx-difficulty-model.json"
)
DEFAULT_EVAL = ROOT / "data/evaluation/tasks.jsonl"
DEFAULT_OUTPUT = ROOT / "data/wlx-grpo"
...
def _training_row(row: dict[str, Any], split: str, index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "prompt": [
            {"role": "system", "content": WLX_SFT_SYSTEM_PROMPT},
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if train_ids & validation_ids or (train_ids | validation_ids) & excluded:
        raise AssertionError("WLX RL task split overlap detected")

    args.output_dir.mkdir(parents=True, exist_ok=True)
    train_path = args.output_dir / "wlx-train.parquet"
    validation_path = args.output_dir / "wlx-validation.parquet"
    metadata_path = args.output_dir / "wlx-metadata.json"
    if any(path.exists() for path in (train_path, validation_path, metadata_path)) and not args.force:
        raise FileExistsError(f"WLX RL data already exists: {args.output_dir}; use --force to rebuild")
