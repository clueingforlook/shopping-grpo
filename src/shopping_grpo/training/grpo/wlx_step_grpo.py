"""Token-level WLX Step-GRPO advantage estimator and veRL runtime hook."""
...
WLX_ADV_ESTIMATOR = "wlx_step_grpo"
...
@register_adv_est(WLX_ADV_ESTIMATOR)
def compute_wlx_step_advantage(
    ...
    **_: Any,
    ...
    for row, info in enumerate(shopping):
        if not isinstance(info, Mapping) or not isinstance(info.get("reward"), Mapping):
            raise ValueError(f"shopping[{row}] is missing WLX reward diagnostics")
        reward = info["reward"]
        if reward.get("version") != "wlx-reward-v4":
            raise ValueError(f"shopping[{row}] does not use WLX Reward v4")
    ...
    with torch.no_grad():
        for uid, rows in group_rows.items():
            ...
            if len(valid_rows) < 2:
                raise ValueError(
                    f"WLX Step-GRPO group {uid!r} has fewer than two valid trajectories"
                )
        ...
        for row in range(batch_size):
            ...
            if not (len(row_spans) == len(row_prm) == len(row_kinds)):
                raise ValueError(f"trajectory {row} has misaligned WLX step fields")
...
def install_wlx_step_grpo() -> None:
    """Patch veRL's narrow dispatch/metrics boundary inside each Ray worker."""
    from verl.trainer.ppo import ray_trainer

    if getattr(ray_trainer.compute_advantage, "_wlx_step_grpo", False):
        return
    ...
    def compute_advantage_with_wlx(
        ...
        if _estimator_name(adv_estimator) != WLX_ADV_ESTIMATOR:
        ...
        advantages, returns = compute_wlx_step_advantage(
    ...
    compute_advantage_with_wlx._wlx_step_grpo = True
