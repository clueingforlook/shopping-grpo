...
class ShoppingToolAgentLoop(ToolAgentLoop):
    ...
    async def _handle_generating_state(
        ...
        start_token = len(agent_data.response_mask)
        next_state = await super()._handle_generating_state(
            agent_data,
            bounded_sampling_params,
            ignore_termination=ignore_termination,
        )
        end_token = len(agent_data.response_mask)
        if runtime_state is not None and end_token > start_token:
            tool_name = None
            parameters = {}
            if next_state == AgentState.PROCESSING_TOOLS and len(agent_data.tool_calls) == 1:
                tool_call = agent_data.tool_calls[0]
                tool_name = str(tool_call.name)
                try:
                    raw_parameters = json.loads(tool_call.arguments or "{}")
                except (TypeError, ValueError, json.JSONDecodeError):
                    raw_parameters = {}
                if isinstance(raw_parameters, dict):
                    parameters = raw_parameters
            runtime_state["model_steps"].append(
                {
                    "index": len(runtime_state["model_steps"]),
    ...
    async def _call_tool(self, tool_call, tools_kwargs, agent_data):
        ...
        if isinstance(step, dict) and state.get("model_steps"):
            model_step = state["model_steps"][-1]
            model_step["environment_step_index"] = step.get("index")
            model_step["progress"] = dict(step.get("progress") or {})
    ...
    async def run(self, sampling_params, **kwargs):
        ...
        try:
            ...
            for model_step in state["model_steps"]:
                start = min(int(model_step["start_token"]), response_length)
                end = min(int(model_step["end_token"]), response_length)
                step_spans.append([start, max(start, end)])
            output.reward_score = reward_score_for_sampling(breakdown, prm)
            output.extra_fields["wlx_step_spans"] = step_spans
            output.extra_fields["wlx_step_prm"] = prm["step_prm"]
            output.extra_fields["wlx_step_rules"] = prm["step_rules"]
            output.extra_fields["wlx_step_kinds"] = prm["step_kinds"]
            output.extra_fields["shopping"] = {
                ...
                "reward_mode": "wlx_v4",
                "reward_version": breakdown["version"],
                "environment_reward_version": state.get("reward_version"),
