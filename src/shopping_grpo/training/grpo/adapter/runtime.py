...
def validate_reward(raw_detail: object) -> dict:
    ...
    preference_scoring = (
        (raw_detail.get("evidence") or {}).get("preference_scoring") or {}
        if isinstance(raw_detail.get("evidence"), Mapping)
        else {}
    )
    raw_dimensions = (
        preference_scoring.get("dimensions") or {}
        if isinstance(preference_scoring, Mapping)
        else {}
    )
    dimension_details = {}
    for name in ("brand", "model", "core_functions", "key_options"):
        raw_dimension = raw_dimensions.get(name) or {}
        if not isinstance(raw_dimension, Mapping):
            raise ValueError(f"dimension detail {name} must be an object")
        try:
            required_count = int(raw_dimension.get("required_count", 0))
            passed_count = int(raw_dimension.get("passed_count", 0))
            verifiable_count = int(raw_dimension.get("verifiable_count", 0))
        except (TypeError, ValueError) as exc:
            raise ValueError(f"dimension detail {name} has invalid counts") from exc
        if (
            required_count < 0
            or not 0 <= passed_count <= required_count
            or not 0 <= verifiable_count <= required_count
        ):
            raise ValueError(f"dimension detail {name} has inconsistent counts")
        dimension_details[name] = {
            "active": bool(raw_dimension.get("active")),
    ...
    option_dimension = raw_dimensions.get("key_options") or {}
    option_results = (
        option_dimension.get("results") or ()
        if isinstance(option_dimension, Mapping)
        else ()
    )
    for result in option_results:
        ...
        if not isinstance(evidence, Mapping):
            continue
        for wrong in evidence.get("wrong_values") or ():
            if not isinstance(wrong, Mapping):
                continue
            selected = str(wrong.get("selected") or "").strip()
            if selected:
                option_failures["wrong_values"].append(
                    {
                        "axis": str(wrong.get("axis") or ""),
                        "selected": selected,
                    }
                )
        for axis in evidence.get("missing_axes") or ():
            axis_text = str(axis or "").strip()
            if axis_text and axis_text not in option_failures["missing_axes"]:
                option_failures["missing_axes"].append(axis_text)
    return {
