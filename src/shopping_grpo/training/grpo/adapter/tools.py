...
def _append_step(state, tool, parameters, done=False, reward=0.0, progress=None):
...
def _public_progress(value):
    ...
    if not isinstance(value, dict):
        return {}
    return {
        "no_progress_steps": int(value.get("no_progress_steps", 0)),
        "runtime_progress_added": [
            str(item) for item in value.get("runtime_progress_added") or ()
        ],
    }
