...
from shopping_grpo.harness.eval_attribution import evaluate_deterministic_attribution
from shopping_grpo.harness.eval_behavior import evaluate_deterministic_behavior
...
from shopping_grpo.harness.eval_eligibility import (
    apply_outcome_review,
    evaluate_eligibility,
)
from shopping_grpo.harness.eval_events import (
    standardize_trajectory_events,
    termination_event_id,
)
from shopping_grpo.harness.eval_outcome import evaluate_outcome
from shopping_grpo.harness.eval_requirements import evaluate_frozen_requirements
from shopping_grpo.harness.tools import ToolRegistry, SFT_TOOL_REGISTRY


TRAJECTORY_EVALUATION_VERSION = EVAL_SCHEMA_VERSION
...
class TrajectoryEvaluation:
    ...
    def to_dict(self) -> dict[str, Any]:
        row = {
            "schema_version": TRAJECTORY_EVALUATION_VERSION,
...
def evaluate_format(
    trajectory: object,
    *,
    tool_registry: ToolRegistry = SFT_TOOL_REGISTRY,
) -> FormatEvaluation:
...
def evaluate_trajectory(
    ...
    *,
    ...
    """生成 Eval v2 确定性面板；语义 Judge 面板明确标为未运行。"""
