...
from shopping_grpo.harness.eval_contracts import EVAL_OUTCOME_VERSION
from shopping_grpo.harness.purchase_verifier import (
    GOLD_PURCHASE,
...
def evaluate_outcome(
    ...
    *,
    ...
    normalized = _normalized_outcome(verification.to_dict(), purchase)
    success = normalized in ENVIRONMENT_SUCCESS_OUTCOMES
    ...
    if environment_success is not None and environment_success != success:
    ...
    outcome = {
        "version": EVAL_OUTCOME_VERSION,
        "normalized_outcome": normalized,
        "purchase_correct": (
            None if normalized == "unverifiable" else bool(success)
        ),
        "purchase_type": (
        ...
        "success_agreement": (
            ...
            else environment_success == success
        ),
