...
from shopping_grpo.harness.contracts import TRAJECTORY_SCHEMA_VERSION, ToolCall
...
def trajectory_to_legacy(trajectory: object) -> dict[str, Any]:
    ...
    if isinstance(schema_version, str) and schema_version.startswith(("wlx-harness-", "harness-")):
        raise TrajectorySerializationError(
            f"unsupported trajectory schema: {schema_version!r}"
        )
