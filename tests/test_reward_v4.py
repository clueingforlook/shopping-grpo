"""Deterministic tests for ORM and conservative PRM events."""
...
def test_orm_gold_is_one():
    ...
    assert compute_orm(state)["terminal_utility"] == 1.0


def test_orm_partial_uses_category_gate_and_requirement_ratios():
    reward = compute_orm(_purchase_state())
...
def test_orm_wrong_category_and_no_purchase_are_ordered():
    wrong_category = compute_orm(_purchase_state(category="fail"))
    no_purchase = compute_orm({"done": False, "reward_type": None})
...
def test_infrastructure_failure_is_masked_not_punished():
    reward = compute_orm({"infrastructure_invalid": True})
...
def test_prm_localizes_wrong_option_and_bad_buy():
    ...
    assert reward_score_for_sampling(compute_orm(state), prm) != compute_orm(state)["terminal_utility"]
