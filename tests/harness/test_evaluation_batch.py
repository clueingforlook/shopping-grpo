"""单模型评测批次测试；不连接模型或 ShopSimulator。"""
...
class EvaluationBatchTest(unittest.TestCase):
    def test_one_model_run_writes_per_task_outputs_and_resumes(self):
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "base-run"
            runner = FakeRunner()
            progress_events = []
            first = asyncio.run(
                run_evaluation_batch(
            ...
            trajectories = _rows(output / "trajectories.jsonl")
            results = _rows(output / "task-results.jsonl")
    ...
    def test_existing_trajectory_is_scored_without_second_rollout(self):
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "resume-run"
            ...
            (output / "task-results.jsonl").write_text("", encoding="utf-8")
            ...
            rows = _rows(output / "task-results.jsonl")
    ...
    def test_incompatible_manifest_is_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "contract-run"
            asyncio.run(
                run_evaluation_batch(
                    task_ids=[1],
                    output_dir=output,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(EvaluationBatchSafetyPause):
                asyncio.run(
                    run_evaluation_batch(
                        task_ids=[1, 2],
                        output_dir=Path(temporary) / "unsafe-run",
                        runner=runner,
                        harness_config=HarnessConfig(max_steps=2),
                        policy_factory=lambda task_id: object(),
                        run_contract={"served_model": "base"},
