...
from shopping_grpo.harness.eval_judge_pipeline import (  # noqa: E402
    JudgeCase,
    run_judge_batch,
)
from shopping_grpo.harness.eval_report import compare_evaluation_runs  # noqa: E402
from shopping_grpo.harness.eval_rubric import freeze_rubric_bundle  # noqa: E402
from shopping_grpo.harness.eval_rubric_generator import (  # noqa: E402
    DeepSeekRubricClient,
)
from shopping_grpo.harness.trajectory_evaluator import evaluate_trajectory  # noqa: E402
...
class OfflinePipelineTest(unittest.TestCase):
    ...
    def test_offline_pipeline_writes_derived_outputs_and_refuses_overwrite(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = root / "run-a" / "trajectories.jsonl"
            source.parent.mkdir()
            source.write_text(
                json.dumps(_trajectory(), ensure_ascii=False) + "\n",
                encoding="utf-8",
            )
            output = source.parent / "eval-v2"
            ...
            evaluations = [
                json.loads(line)
                for line in (output / "trajectory-evaluations.jsonl")
