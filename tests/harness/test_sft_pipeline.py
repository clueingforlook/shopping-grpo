"""离线验证 SFT 采样、难度、清洗和 Teacher 策略的主要安全规则。"""
...
from shopping_grpo.harness.config import ContextPolicy, HarnessConfig, ObservationPolicy  # noqa: E402
from shopping_grpo.harness.contracts import ModelRequest  # noqa: E402
from shopping_grpo.harness.sft_contracts import (  # noqa: E402
    SamplingConfig,
    SamplingMode,
    SftTask,
)
from shopping_grpo.harness.sft_dataset import (  # noqa: E402
    DatasetBuildConfig,
...
from shopping_grpo.harness.sft_outcomes import classify_attempt  # noqa: E402
from shopping_grpo.harness.sft_pipeline import default_sft_harness_config  # noqa: E402
from shopping_grpo.harness.sft_policy import OpenAICompatibleTeacherPolicy  # noqa: E402
from shopping_grpo.harness.sft_sampler import (  # noqa: E402
    AttemptExecution,
...
from shopping_grpo.harness.sft_storage import RawTrajectoryStore, read_jsonl  # noqa: E402
SCRIPT_PATH = REPOSITORY / "scripts" / "sft_data_pipeline.py"
SCRIPT_SPEC = importlib.util.spec_from_file_location(
    "sft_data_pipeline_test_cli",
    SCRIPT_PATH,
)
...
class DifficultyTest(unittest.TestCase):
    ...
    def test_fit_cli_binds_verified_provenance_and_reports_it(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            model = Mock(difficulty_version="difficulty-test")
            ...
            stored_report = json.loads(
                (output_dir / "difficulty-report.json").read_text(encoding="utf-8")
            )
...
class TeacherPolicyTest(unittest.IsolatedAsyncioTestCase):
    """检查 Teacher 策略会记录 Usage 且与 Core 配置一致。"""
    ...
    async def test_policy_records_each_chat_completion_usage(self):
        ...
        with patch(
            "shopping_grpo.harness.sft_policy.asyncio.to_thread",
    ...
    def test_policy_logs_retry_status_without_request_or_key(self):
        ...
        with patch("shopping_grpo.harness.sft_policy.time.sleep") as sleep:
