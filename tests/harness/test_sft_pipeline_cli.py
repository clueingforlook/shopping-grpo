"""离线检查 SFT 分阶段命令行，不连接模型或 ShopSimulator。"""
...
def _load_cli_module():
    ...
    path = REPOSITORY / "scripts" / "sft_data_pipeline.py"
    spec = importlib.util.spec_from_file_location("sft_data_pipeline_cli", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 SFT CLI")
