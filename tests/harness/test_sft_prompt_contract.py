"""离线锁定 SFT 的 Prompt、工具和首轮 DeepSeek 请求契约。"""
...
from shopping_grpo.harness.contracts import EpisodeRequest, ErrorCategory  # noqa: E402
...
class SftPromptContractTest(unittest.IsolatedAsyncioTestCase):
    ...
    def test_prompt_and_registry_have_one_exact_contract(self) -> None:
        ...
        self.assertEqual(SFT_TOOL_SCHEMAS, canonical_without_removed_function)
        self.assertEqual(SFT_TOOL_REGISTRY.tool_names, EXPECTED_SFT_TOOLS)
        self.assertNotIn("think", SFT_SYSTEM_PROMPT.lower())
        for name in EXPECTED_SFT_TOOLS:
            self.assertIn(f"`{name}`", SFT_SYSTEM_PROMPT)
        for legacy_action in ("search[", "click[", "finish["):
            self.assertNotIn(legacy_action, SFT_SYSTEM_PROMPT)
        self.assertIn('`{"reason":"no_suitable_product"}`', SFT_SYSTEM_PROMPT)
        self.assertTrue(SFT_SYSTEM_PROMPT_VERSION.startswith("wlx-sft-"))
        self.assertEqual(len(SFT_SYSTEM_PROMPT_SHA256), 64)

        for schema in SFT_TOOL_SCHEMAS:
        ...
        with self.assertRaisesRegex(ToolRegistryError, "unknown tool"):
            SFT_TOOL_REGISTRY.parse_tool_call(
                {
                    "id": "removed",
                    "type": "function",
                    "function": {
    ...
    def test_resume_rejects_a_raw_directory_from_the_old_prompt(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            config_path = Path(temporary) / "collection-config.json"
    ...
    def test_custom_runner_cannot_bypass_the_sft_contract(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "SFT_SYSTEM_PROMPT"):
            SftDataPipeline(
                harness_config=config,
                runner=EpisodeRunner(
        ...
        pipeline = SftDataPipeline(harness_config=config)
