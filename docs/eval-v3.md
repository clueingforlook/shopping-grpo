...
`eval-v3` 用 ShopSimulator 的结构化终局记录评价一个指定模型，不调用 LLM Judge，也不使用旧 Rubric。一次可以只运行 Base、SFT 或 GRPO 中的一种。
...
- `eval-v3-manifest.json`：模型、任务集、环境和运行配置指纹；
- `eval-v3-task-results.jsonl`：逐任务 Gold 和各结构化分项；
- `eval-v3-summary.json`：总体及分难度统计；
- `eval-v3-summary.md`：便于阅读的简表。

跨模型比较是后续可选步骤，不属于单模型 Eval v3 的必需产物。

## 8. 实现与启动

入口脚本：`scripts/run_eval_v3.py`。脚本、文件和新输出目录按用途命名；冻结结果中的模型名、协议版本和来源记录保留历史标识。

### 8.1 只评 GRPO

先从 checkpoint 提取独立 LoRA。当前 step 500 已提取到
`outputs/models/grpo-step500-lora`。使用冻结的 SFT merged 模型作为底座，
并显式加载 GRPO LoRA：

```bash
bash scripts/serve_evaluation_model.sh \
  --base-model outputs/models/sft-own-data-v1-merged \
  --lora-adapter outputs/models/grpo-step500-lora \
  --served-model grpo-step500
```

环境与模型服务启动后，一条命令运行 Final-200 并生成 Eval v3：

```bash
.venv/bin/python scripts/run_eval_v3.py run \
  --model-role grpo \
  --model-name grpo-step500-lora \
  --served-model grpo-step500 \
  --model-artifact outputs/models/grpo-step500-lora \
  --tokenizer outputs/models/sft-own-data-v1-merged \
  --output-dir outputs/evaluation/eval-v3/grpo-step500-lora
```
...
```bash
less outputs/evaluation/eval-v3/grpo-step500-lora/eval-v3-summary.md
```

逐任务结果保存在 `eval-v3-task-results.jsonl`，总体和分难度统计同时保存在 `eval-v3-summary.json`。
...
```bash
.venv/bin/python scripts/run_eval_v3.py score \
  --model-role sft \
  --model-name wlx-sft-final200-v2 \
  --trajectories outputs/evaluation/sft-final200-v2/trajectories.jsonl \
  --run-manifest outputs/evaluation/sft-final200-v2/run-manifest.json \
  --output-dir outputs/evaluation/eval-v3/sft
```
...
```bash
.venv/bin/python scripts/plot_grpo_metrics.py \
  --log outputs/models/rl-v4-main-24k-noentropy/train.log
```

默认输出到 `training-plots/`：

- `training-curves.svg`：ORM、Gold、KL、梯度、有效组和响应长度曲线；
- `training-metrics.csv`：每个训练 step 的原始指标；
- `validation-metrics.csv`：固定 validation 的 Gold、ORM 和轮数；
- `training-curve-summary.json`：最后一个训练点和验证点。
