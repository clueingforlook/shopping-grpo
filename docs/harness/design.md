...
> 状态：v1 已实现，最初作为旁路架构加入。当前代码已纳入 `shopping_grpo.harness` 包，基础 SFT、GRPO、Evaluation 入口继续保留。本文记录核心契约，完整项目状态见[文档导航](../README.md)。

## 1. 目标与边界

Harness 把购物 Agent 各阶段共有的运行约束放进统一底座，各阶段只保留自己的数据或训练逻辑。
...
过滤、清洗、切分、落盘      指标、汇总、Judge          token、logprob、优化
       │                        │                         │
       ▼                        ▼                         ▼
stage_sft.py       stage_evaluation.py    stage_grpo.py
       └───────────────┬───────────────┬─────────────────┘
                       ▼
                 Harness Core
...
| 文件 | 责任 |
|---|---|
| [contracts.py](../../src/shopping_grpo/harness/contracts.py) | 请求、工具调用、Step、Trajectory、错误和终止类型 |
| [config.py](../../src/shopping_grpo/harness/config.py) | Core、上下文、Observation 配置及参数校验 |
| [tools.py](../../src/shopping_grpo/harness/tools.py) | 单一 Tool Schema 注册表、Guard、环境动作转换、指纹校验 |
| [environment.py](../../src/shopping_grpo/harness/environment.py) | `ShopAgentEnv` 的异步生命周期与版本校验 |
| [policy.py](../../src/shopping_grpo/harness/policy.py) | 现有 `complete(messages, tools)` 客户端适配 |
| [reward.py](../../src/shopping_grpo/harness/reward.py) | Reward v3 结构和语义校验 |
| [runner.py](../../src/shopping_grpo/harness/runner.py) | SFT/Evaluation 共用的消息级 Episode 状态机 |
| [serialization.py](../../src/shopping_grpo/harness/serialization.py) | Core JSON 与现有 legacy trajectory 的显式转换 |
| [stage_sft.py](../../src/shopping_grpo/harness/stage_sft.py) | SFT 验收、held-out 防泄漏和 artifact 构造入口 |
| [stage_evaluation.py](../../src/shopping_grpo/harness/stage_evaluation.py) | 归一化、确定性指标与汇总入口 |
| [stage_grpo.py](../../src/shopping_grpo/harness/stage_grpo.py) | HarnessConfig 到 veRL AgentLoop/Tool/metadata 的桥接 |

Python 包路径是 `shopping_grpo.harness`，源码位于 `src/shopping_grpo/harness/`；模块按职责命名。配套[使用说明](usage.md)与本设计文档位于 `docs/harness/`。
...
- `trajectory_to_legacy(mapping)`：落盘后把 Core JSON 显式转成 legacy；未知 schema 会 fail-fast。
...
该基础阶段适配器复用现有 SFT 构建逻辑。它只接受完整的 Reward v3 `gold_purchase`，清除私有 reasoning/终局 Reward 文本，并强制 held-out 隔离。自研完整采集与难度校准流程另见 [SFT 数据流水线](../sft-data-pipeline.md)。
...
verl_overrides = bridge.to_verl_overrides(
    ...
    tool_config_path="configs/harness_tools.json",
)
```
...
当前 Harness 已随项目包一起安装。在仓库根目录准备基础开发环境：

```bash
uv sync --extra dev
```

直接运行未安装的源码时，可设置 `PYTHONPATH=src`。基础入口与自研流水线均保留，按[脚本索引](../../scripts/README.md)选择相应阶段。
