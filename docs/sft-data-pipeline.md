...
我们要建设的不是一个单独的“SFT 采样函数”，而是一条完整的 **SFT 数据采集流水线**。

其中，`src/shopping_grpo/harness/` 只是中间负责“把一条轨迹正确跑出来”的发动机。任务选择、难度配比、数据清洗等工作不应该全部塞进 Core。
...
- 由 的 Environment Adapter 连接 ShopSimulator；
- Teacher、任务选择、清洗、配比全部通过 自己的 SFT 流水线完成。
...
| 5. 采样调度 | 负责并发、重试、断点续采和每题采样次数 | 属于 SFT 阶段层 |
...
Harness Core 必须保证 Teacher 看不到下面这些环境内部信息：
...
在线轨迹采样是 `src/shopping_grpo/harness/` 的主要职责。Core 应负责：
...
outputs/data-v1/
├── raw/
│   ├── calibration-200/  # 200 个校准任务：canonical 942 行 + 历史补充 49 行
│   ├── formal/           # 后续六批正式采样：2,389 行
│   └── records.jsonl     # 全部 3,380 条轨迹的逐条结果记录
└── sft/
    ├── sft.jsonl         # 最终 428 条 SFT；train/validation 为 385/43
    └── records.jsonl     # 3,331 条候选轨迹的接收/拒绝记录
```

`records.jsonl` 逐条记录成功、失败、技术异常和 SFT 去留；各目录的 `manifest.json` 保存来源、计数与 SHA-256，采样配置和计划放在 `provenance/`。
...
```mermaid
flowchart TB
    A["SFT 任务与数据策略层<br/>选任务、泄漏、难度、配比"] --> B["SFT 采样编排层<br/>并发、重试、断点续采"]
    B --> C["Harness Core<br/>把一条轨迹安全完整地跑出来"]
    C --> D["SFT 数据构建层<br/>清洗、长度、去重、配比、切分"]
    D --> E["最终 SFT Train / Validation 数据"]
```

各层职责可以概括为：

- `src/shopping_grpo/harness/`：保证一条轨迹运行正确、安全、可审计；
- SFT 任务与采样层：决定拿哪些任务调用 Core，以及每个任务采几次；
- SFT 数据构建层：决定哪些轨迹进入训练，以及最终的数据比例和切分方式。

## 13. 第一版实现位置

第一版已经新增独立的 SFT 路径，不调用原项目的 `collect_for_task` 或 `collection/sft.py`：

| 文件 | 作用 |
|---|---|
| `sft_contracts.py` | 任务、尝试结果、采样配置等公共数据格式 |
| `sft_outcomes.py` | 区分有效尝试、任务成功与是否进入 SFT |
| `sft_sampler.py` | 校准/正式调度、并发、技术重试和断点续采 |
| `sft_storage.py` | 追加保存 `raw.jsonl`、泄漏检查和文件哈希 |
| `sft_metrics.py` | Token、有效步骤、唯一搜索词和长度桶 |
| `sft_difficulty.py` | C/R/N 规则分数和纯 Python 逻辑回归校准 |
| `sft_shopsim_difficulty.py` | 复用 ShopSimulator BM25 与 Reward-v3 预计算 R/N |
| `sft_dataset.py` | 严格 Gold 清洗、配比、分层切分、导出与冻结 |
| `sft_deepseek_v4.py` | 按 DeepSeek V4 官方 Encoding 渲染工具调用和 Thinking 上下文 |
| `sft_tokenizer.py` | 用轻量 Teacher tokenizer 计数，并用训练 tokenizer 做最终精确计数 |
| `sft_policy.py` | 自己的 OpenAI-compatible Teacher 接口 |
| `sft_pipeline.py` | 把上面模块串起来的 SFT 阶段总入口 |

现有旧文件未在本次整理中删除，但 SFT 的唯一正式入口是 `SftDataPipeline`；本次没有新增兼容启动器，也不会落回原项目 Harness。
...
第一版实现时记录的离线回归结果：105 项通过，ShopSimulator 商品页 2 项通过。这是当时的验证记录，当前验证状态以本次测试输出为准。

## 14. 第一版冻结状态

> 以下保留第一版数据冻结时的记录。项目后续已完成 SFT、GRPO 和评测，见[实验概览](experiments.md)；本地公开 SFT 快照的 379/49 划分与此处早期 385/43 划分分别说明于[数据说明](../data/README.md)。
...
- 数据入口：`outputs/data-v1/`，其中 Raw、SFT、逐条判定记录和 provenance 均已保存并核对 SHA-256。
